...
def nudge_codex_cli() -> bool:
    ...
    ので付けない。`--skip-git-repo-check` は必須: cwd の STATE_DIR は意図的に
    git リポジトリ外なので、現行 CLI の信頼済みディレクトリチェックに必ず引っかかる
    （「Not inside a trusted directory」も実測。素の `exec` も同じ理由で落ちる）。
    フラグ名は今後も版で変わりうるので、弾かれたら素の `exec` でもう一度試す
    ...
    try:
        ...
        pass
    variants = [
        ["exec", "--sandbox", "read-only", "--skip-git-repo-check", CODEX_REFRESH_PROMPT],
