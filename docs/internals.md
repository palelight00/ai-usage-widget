...
- `window_minutes` で枠の種類を判定する（`300` = 5 時間、`10080` = 週次）。
  ...
  枠の長さ順に並べ直す）。
- 巨大なファイル（数十 MB）があるので末尾 3 MB だけ読む。走査は「新しい順に
  12 ファイル」ではなく、**有効なイベントが取れたファイルを 12 件集めるまで**続ける
  （上限 `CODEX_SCAN_FILES_MAX` = 120 ファイル）。
- **デスクトップアプリはターンを伴わないセッションのスタブを一斉に作ることがある**
  （2026-08-27 実測: 深夜 1:26 に 12 ファイルが同一秒で生成。どれも `token_count`
  イベントを 1 行持つが `rate_limits` は **null**）。単純なファイル数の窓だと
  この一斉生成だけで埋まり、実イベントを持つ古いファイルを見失う（実際に
...
- **openai/codex の開発版は 7 日より古い rollout を `.zst` に圧縮する**（mtime 基準・
  ...
  1 週間ぶんだけ**になる。ただし**作者の環境（同日）では 2026-06 の rollout が
  未圧縮のまま**で、インストール版にはまだ入っていない模様。つまり現時点の
  「イベントなし」は圧縮ではなく「最近ターンを回していない」ほうが原因。
