...
- **`codex exec` で回した分の rollout は自分で掃除する。**1 回叩くと
  `~/.codex/sessions/` に 1 本増えるので、放っておくと `codex resume` の一覧が
  「`ok` としか言わないセッション」で埋まる（実際にそうなった）。叩いた回の最後に、
  **`cwd` が `~/.ai-usage` の rollout だけ**を新しいほうから 1 本残して消す。
  nudge は `cwd=~/.ai-usage` で回しているので、この cwd で始まったセッションは
  自分の分しかない。1 本残すのは、**CLI はターンを回せたのにこちらの再取得が
  失敗した場合**に、その rollout がいちばん新しい `rate_limits` を持っているから。
  溜まった分は `--prune-codex-sessions`（`--dry-run` で対象だけ見られる）。
  **`~/.ai-usage` で codex を手で回さないこと**（掃除に巻き込まれる）。
...
- ~~Codex のトークン失効時の自動復旧~~ → 実装済み（`nudge_codex_cli()`）。
  ...
  2026-08-22、**予定どおり失効して実地で動き始めた**。`~/.ai-usage` を cwd とする
  「`ok` としか言わないセッション」が 1 日 1 本ずつ増えているのを利用者が見つけた
  （＝ nudge が本当に回っている）。1 時間に 1 回まで絞ってあるのに 1 日 1 本という
  ことは、**叩いた回はそのつど復旧できている**（復旧できていなければ毎時 1 本ずつ
  増えるはず）。ただし **1 日で切れる理由は未確認**で、当初の「約 10 日」とは合わない。
  リフレッシュ後のトークンの寿命が短い可能性があるので、ログの `codex_exp` の推移で
  確かめること。残っていた rollout は掃除するようにした（「決まっていること」を見よ）。
