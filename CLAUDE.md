...
- **Codex のトークン失効時の復旧経路。**Claude は 401 のとき `claude -p` を 1 回
  呼んで公式に更新させるが、**Codex には同等の仕組みが無い**（`fetch_codex_raw()` は
  `auth.json` を読むだけ）。切れると手動で再ログインするまで API が落ち続け、
  JSONL の古い値を配り続ける。2026-08-12 に 183 時間の失効を実際に踏んだ。
  `codex` CLI に認証を通させる経路があるか未調査。
