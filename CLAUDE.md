...
- **現行の codex は 7 日より古い rollout を `.zst` に圧縮する**（2026-08-26 に
  openai/codex のソースで確認。mtime 基準・起動時のバックグラウンドジョブ）。
  スクリプトは素の `.jsonl` しか読まないので、**フォールバックが効くのは直近
  1 週間ぶんだけ**。`--raw` の「イベントなし」はこれが主因になりうる。行フォーマットと
