...
入力を求めるのは**困っているときだけ**にしてある。正常に動いているのに毎回聞かれると
邪魔になるため、アプリ内で実行しても次の場合以外は黙って表示だけする。

- まだ登録されていない
- 登録済みだが取得できない（リンク切れ、共有設定の誤り）

**動いているリンクを変えたいとき**は、ショートカットの `Run Script` でパラメータに
`setup` を渡して `AIUsage` を実行する。現在の値が入った入力欄が出る。
空にして保存すれば登録が消え、iCloud 経由に戻る。
（ウィジェットからは画面を出さないので、自動実行の邪魔にはならない）
